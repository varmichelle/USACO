...
public class badmilk {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		PrintStream out = new PrintStream(new File("badmilk.out.txt"));
		...
		for (int i = 1; i <= num_milk_types; i++) {
			boolean is_bad = true;
			// loop through each sick person
			for (int j = 0; j < num_sick; j++) {
				boolean satisfied = false;
				// loop through each milk tasting
				for (int k = 0; k < num_tastings; k++) {
					int person = sick[j][0];
					int time = sick[j][1];
					int milk_type = i;
					if (tastings[k][0] == person && tastings[k][2] < time && tastings[k][1] == milk_type) satisfied = true;
				}
				if (satisfied == false) {
					is_bad = false;
					break;
