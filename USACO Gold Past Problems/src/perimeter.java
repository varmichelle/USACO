...
public class perimeter {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		PrintStream fileWriter = new PrintStream(new File("perimeter.out.txt"));
		...
		for (int i = 0; i < numBayhales; i++) {
			...
			if (coord[i][0] > largest) largest = coord[i][0];
			coord[i][1] = fileReader.nextInt();
			if (coord[i][1] > largest) largest = coord[i][1];
		}
		grid = new char[largest + 2][largest + 2];
		for (int i = 0; i < numBayhales; i++) grid[coord[i][0]][coord[i][1]] = '*';
	...
	public static void perimeter(int largest, char[][] grid) {
		for (int x = 0; x < largest + 2; x++) {
			for (int y = 0; y < largest + 2; y++) {
				if (grid[x][y] == '.') {
					// check left
					if (x > 0 && grid[x - 1][y] == '*') totalPerimeter++;
					// check right
					if (x < largest + 1 && grid[x + 1][y] == '*') totalPerimeter++;
					// check up
					if (y > 0 && grid[x][y - 1] == '*') totalPerimeter++;
					// check down
					if (y < largest + 1 && grid[x][y + 1] == '*') totalPerimeter++;
				}
			}
		}
	}
	...
	public static void floodfill(int x, int y, int largest) {
		if (x < 0 || y < 0 || x > largest + 1 || y > largest + 1) return;
		else {
